...
import {
  ...
  getToc,
  search,
} from "../src/sources/legislacao/query";
import { EmbedderStub } from "./support/embedder-stub";

const TestLayer = Layer.mergeAll(DbLayer, EmbedderStub);
...
describe("legislacao store + query", () => {
  it.effect("hybrid search ranks results and honors the norma filter", () =>
    Effect.gen(function* () {
      yield* seed;
      const hits = yield* search("licitacoes", { normaId });
  ...
  it.effect("an unknown norma filter returns no hits", () =>
    Effect.gen(function* () {
      yield* seed;
      const hits = yield* search("licitacoes", { normaId: "inexistente" });
      expect(hits).toEqual([]);
    }).pipe(Effect.provide(TestLayer))
  );

  it.effect("getToc returns the tree ordered by position from the root", () =>
    Effect.gen(function* () {
      yield* seed;
      const toc = yield* getToc(normaId);
      expect(Number(toc[0].depth)).toBe(1);
      expect(toc[0].parent_path).toBeNull();
  ...
  it.effect("getArticle resolves a nested article by number", () =>
    Effect.gen(function* () {
      yield* seed;
      const res = yield* getArticle(normaId, "lei_14133_2021", 17);
  ...
  it.effect("getArticle fails NODE_NOT_FOUND for an unknown article", () =>
    Effect.gen(function* () {
      yield* seed;
      const error = yield* Effect.flip(getArticle(normaId, "lei_14133_2021", 999));
      expect(error._tag).toBe("LegislacaoError");
      expect(error.message).toContain("Trecho nao encontrado");
    }).pipe(Effect.provide(TestLayer))
  );

  it.effect("getNode surfaces the structure summary", () =>
    Effect.gen(function* () {
      yield* seed;
      const res = yield* getNode("lei_14133_2021.tit_ii.cap_i.art17");
      expect(String(res.node.summary)).toContain("Art. 17");
    }).pipe(Effect.provide(TestLayer))
  );

  it.effect("getNode resolves breadcrumb and direct children", () =>
    Effect.gen(function* () {
      yield* seed;
      const res = yield* getNode("lei_14133_2021.tit_ii.cap_i.art17");
      expect(res.node.kind).toBe("artigo");
  ...
  it.effect("ltree subtree query returns only descendants of art17", () =>
    Effect.gen(function* () {
      yield* seed;
      const db = yield* Db;
