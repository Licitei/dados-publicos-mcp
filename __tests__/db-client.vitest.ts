...
describe("kernel/db", () => {
  it.effect("round-trips an insert and select through effect-postgres drizzle", () =>
    Effect.gen(function* () {
      const db = yield* Db;
      yield* db.execute(
  ...
  it.effect("a failing query surfaces a drizzle query error in the error channel", () =>
    Effect.gen(function* () {
  ...
  it.effect("loads pgvector and exposes cosine distance", () =>
    Effect.gen(function* () {
      const db = yield* Db;
  ...
  it.effect("loads ltree and resolves subtree ancestry", () =>
    Effect.gen(function* () {
      const db = yield* Db;
  ...
  it.effect("loads pg_textsearch and ranks a bm25 match in pt-BR", () =>
    Effect.gen(function* () {
      const db = yield* Db;
