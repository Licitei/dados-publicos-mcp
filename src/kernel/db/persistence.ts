  ...
  return Match.value(process.platform).pipe(
    ...
    Match.orElse(() =>
      xdgDataHome.pipe(
        Option.orElse(() =>
          home.pipe(
            Option.map((dir) => path.join(dir, ".local", "share"))
          )
        ),
        Option.map((dir) => path.join(dir, appDirName)),
        Option.getOrElse(() => path.join(".", appDirName))
...
const resolveDataDir = Effect.gen(function* () {
  const explicit = yield* envOption("DADOS_PUBLICOS_MCP_DATA_DIR");
  const dir = yield* Option.match(explicit, {
    onNone: () => baseDir,
...
export const DbPersistenceLive = Layer.effect(DbConfig)(
  resolveDataDir.pipe(Effect.map((dataDir) => ({ dataDir })))
);
