...
export function registerDadosPublicosPrompts(server: McpServer) {
  server.registerPrompt(
    ...
    (args) => promptResult("Analisar edital", [
  ...
  server.registerPrompt(
    ...
    (args) => promptResult("Analisar orgao", [
  ...
  server.registerPrompt(
    ...
    (args) => promptResult("Encontrar atas RP", [
  ...
  server.registerPrompt(
    "check_supplier",
    {
      ...
      description: "Roteiro para verificacao publica basica de fornecedor por CNPJ.",
      argsSchema: { cnpj: z.string() },
    },
    (args) => promptResult("Checar fornecedor", [
